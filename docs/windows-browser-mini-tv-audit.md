...
`v1.13.0.20` 已落实本审计建议的第一阶段：
- 浏览器文本结果与截图结果都能在 `Minis Computer` 展示
- 清理跨工具调用遗留的旧截图状态
- 工具步骤保存真实 URL / 命令，并按后端结果标记成功或失败
- 工具输出与错误文本改用 UTF-8 安全截断，避免中文边界触发 Rust panic
- Windows CI 构建与 Release 分离，版本从源码推导并校验

原版完整浏览器动作集仍属于后续功能对齐范围。
